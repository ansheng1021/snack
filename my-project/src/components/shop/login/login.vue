<template>
    <div class="login">
        <div class="content">
            <header>
                <img src="../../admin/static/img/11.jpg" alt="">
               <h1>商家登录</h1>
            </header>
            <div class="myfrom">
                ...
                <span style="margin:0 10px 0 17px">保存信息</span><span>忘记密码</span>
                    ...
                    <el-form-item>
                        <el-button round @click="onsubmit('form')">登录</el-button>
                         <router-link :to="{name:'admissionindex'}" tag="el-button">入驻</router-link>
                    </el-form-item>
            </el-form>
            </div>
        </div>
        ...
        methods:{
            onsubmit(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.axios.post('/shop/login/check',this.form).then(res=> {
...
<style scoped>
    ...
    .login{
        ...
        bottom: 0;
        right: 0;
        background: url("../../admin/static/img/bg.png");
